...
|---|---|---|
| [`DeployCore.s.sol`](DeployCore.s.sol) | `PayRegistry`, `VirtContractResolver`, `AgentPayWallet` | Once per network. Permanent — never redeployed. |
| [`DeployLedger.s.sol`](DeployLedger.s.sol) | `AgentPayLedger` (+ 3 ledger libraries; see below) | Versioned. Run again for each new ledger version; peers cooperatively migrate. |
...
`AgentPayLedger` is split across three Solidity `library` contracts under
...
1. The Solidity compiler emits **20-byte zero placeholders** in `AgentPayLedger`'s
   bytecode wherever a library function is called, recording each placeholder offset
   in `out/AgentPayLedger.sol/AgentPayLedger.json` under `bytecode.linkReferences`.
2. When `forge script` evaluates `new AgentPayLedger(...)`, it walks the link references,
   **deploys each library as its own contract** (one transaction each), and **patches
   the placeholders** in `AgentPayLedger`'s bytecode with the freshly-deployed library
   addresses.
3. The now-linked `AgentPayLedger` is deployed last.
...
`LedgerMigrate`, then `AgentPayLedger`. All four appear in the broadcast log under
`broadcast/DeployLedger.s.sol/<chainId>/run-latest.json` (see
[Broadcast outputs](#broadcast-outputs) below).

Library calls happen via `DELEGATECALL`, so the libraries execute in `AgentPayLedger`'s
storage context — they're code on a separate address but operate on `AgentPayLedger`'s
state.
...
addresses and link `AgentPayLedger` against them at compile time, add to `foundry.toml`:
...
`forge script` emits exactly **one** transaction (`new AgentPayLedger(...)`) instead of
...
Peer-controlled migration means a new `AgentPayLedger` is just a fresh deploy pointing at
