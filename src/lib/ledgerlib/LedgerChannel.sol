...
library LedgerChannel {
        ...
        returns (address[2] memory, uint256[2] memory, uint256[2] memory)
    {
        ...
        uint256[2] memory withdrawals = [_c.peerProfiles[0].withdrawal, _c.peerProfiles[1].withdrawal];
    ...
    function _validateSettleBalance(LedgerStruct.Channel storage _c) internal view returns (bool, uint256[2] memory) {
        ...
        uint256[2] memory settleBalance = [
            ...
            peerProfiles[1].deposit + peerProfiles[0].state.transferOut
        ];
        for (uint256 i = 0; i < 2; i++) {
            uint256 subAmt = peerProfiles[i].state.transferOut + peerProfiles[i].withdrawal;
            if (settleBalance[i] < subAmt) {
                return (false, [uint256(0), uint256(0)]);
            }

            settleBalance[i] = settleBalance[i] - subAmt;
        }
